              ...
              <div className="product-list">
                {category.plants.map((plant, plantIndex) => (
                  <div className="product-card" key={plantIndex}>
                    ...
                    <button style={{backgroundColor: addedToCart[plant.name] ? "gray" : "#615EFC",}}disabled={addedToCart[plant.name] ? true : false} className="product-button" onClick={() => addedToCart[plant.name] ? handleRemoveFromCart(plant) : handleAddToCart(plant)}>
                    Add to Cart
                    </button>
                  </div>
                ))}
